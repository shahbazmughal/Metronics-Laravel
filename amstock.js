(function () {
	...
	e.AmStockChart = e.Class({
		...
		getFirstDate: function (a) {
			...
			return new Date(e.resetDateToMin(a, this.categoryAxesSettings.minPeriod, 1, this.firstDayOfWeek))
		},
		updateData: function () {
			var a = this,
				b = a.mainDataSet;
		...
		getEventDataItem: function (a, b, c, d, k, f) {
			...
			if (a >= q && a < l) return g;
			if (!(1 >= k - d)) return a < q ? this.getEventDataItem(a, b, c, d, n,
				f) : this.getEventDataItem(a, b, c, n, k, f)
		},
			...
			if (0 < this.dataSets.length) {
				...
				for (b = 0; b < a.length; b++) {
					...
					for (l = 0; l < f.length; l++) {
							...
							if (g.valueAxis) {
								...
								for (D = 0; D < B; D++) {
									...
									if (m) {
										...
										p && (t && y ? (r.legendValueText = y, r.legendPeriodValueText = A) : (v && (r.legendValueText = v), r.legendPeriodValueText = E));
						...
						p && (t && y ? (g.legendValueText = y, g.legendPeriodValueText = A) : (v && (g.legendValueText = v), g.legendPeriodValueText = E))
					}
				}
			}
		},
	...
	e.PeriodSelector = e.Class({
		...
		write: function (a) {
			...
			if (b.inputFieldsEnabled) {
				...
				n.appendChild(g);
				f ? e.addBr(n) : (n.style.styleFloat = "left", n.style.display = "inline");
				var m = document.createElement("input");
				m.className = "amChartsInputField " + d + "-start-date-input";
				k && e.applyStyles(m.style, k.PeriodInputField);
				...
				n.appendChild(m);
				b.endDateField = m;
				f ? e.addBr(n) : l = m.offsetHeight + 2;
				g && (m.style.width = g)
			}
			n = b.periods;
			if (e.ifArray(n)) {
				g = document.createElement("div");
				f || (g.style.cssFloat = "right", g.style.styleFloat = "right", g.style.display = "inline");
				a.appendChild(g);
				f && e.addBr(g);
				...
				b.periodContainer = g;
				var t;
				for (a = 0; a < n.length; a++) c = n[a], t = document.createElement("input"), t.type = "button", t.value = c.label, t.period = c.period, t.count = c.count, t.periodObj = c, t.className = "amChartsButton " + d + "-period-input", k && e.applyStyles(t.style, k.PeriodButton), f && (t.style.width = b.width - 1 + "px"), t.style.boxSizing = "border-box", g.appendChild(t), b.addEventListeners(t), c.button = t;
				!f && t && (h = t.offsetHeight)
			}
			b.offsetHeight = Math.max(l, h)
		},
